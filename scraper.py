...
# ═══════════════════════════════════════════════════════
#  PLAYWRIGHT FETCH (handles JS challenges like InfinityFree/wuaze)
# ═══════════════════════════════════════════════════════

async def _fetch_with_playwright(url: str) -> Optional[tuple]:
    """
    Use Playwright headless Chromium to bypass JS challenges like InfinityFree/wuaze.
    Raises exception on failure so fetch_with_fallback shows the real error.
    """
    try:
    ...
    try:
        async with async_playwright() as pw:
            ...
            # Step 1: load the page (JS challenge runs automatically)
            try:
                response = await page.goto(url, wait_until="domcontentloaded", timeout=30000)
            except PWTimeout:
                raise Exception("Playwright: page.goto timeout (30s)")

            # Step 2: wait for InfinityFree JS challenge to complete + redirect
            await page.wait_for_timeout(4000)

            # Step 3: wait for the real page to finish loading
            try:
                await page.wait_for_load_state("networkidle", timeout=20000)
            except PWTimeout:
                pass  # OK, grab whatever is loaded

            final_url = page.url
            html = await page.content()
            status = response.status if response else 200

            logger.info(f"Playwright OK: {final_url} size={len(html)} status={status}")
            await browser.close()

            if not html or len(html) < 200:
                raise Exception(f"Playwright: got empty page (size={len(html)})")

            low = html.lower()
            # Still on challenge page?
            if ("cheungid" in low or "tochk" in low) and len(html) < 2000:
                raise Exception("Playwright: still on challenge page after wait")

            return html, status, "text/html", final_url
        ...
        if browser:
            try:
                ...
                pass
        raise Exception(f"Playwright: {e}")
...
class AdvancedFetcher:
    """
    Tries 8 strategies to fetch a URL:
    0. Playwright headless browser (JS execution — bypasses ALL JS challenges)
    1. InfinityFree cookie math bypass (fast fallback)
    2. Direct with SSL verification
    3. Direct WITHOUT SSL
    4. With cookies
    5. Rotate all user agents
    6. Google Cache
    7. Wayback Machine
    """
    ...
    async def fetch_with_fallback(
        ...
        """
        Returns: (html, status_code, content_type, final_url, method_name)
        """
        strategies = [
            ("playwright",          self._pw_fetch,                {}),
            ("infinityfree_cookie", self._fetch_infinityfree_bypass, {}),
            ("direct_ssl",          self._fetch_direct, {"ssl_verify": True}),
            ("direct_no_ssl",       self._fetch_direct, {"ssl_verify": False}),
        ...
        for name, func, kwargs in strategies:
            ...
            try:
                ...
                if result and result[0] and len(result[0]) > 200:
                    ...
                    errors[name] = "returned empty/None"
                    logger.warning(f"❌ {name} returned empty for {url}")
            except Exception as e:
        ...
        return None, 0, "", url, f"all_failed:\n{err_lines}"

    async def _pw_fetch(self, url: str) -> Optional[tuple]:
        """Wrapper to call the module-level playwright fetch."""
        return await _fetch_with_playwright(url)

    @staticmethod
    def _solve_infinityfree_token(html: str):
        """Solve InfinityFree JS challenge cookie math."""
        import re as _re
        vars_found = {}
        ...
        m = _re.search(r'document\.cookie\s*=\s*"__test="\s*\+\s*\((\d+)\s*\*\s*(\d+)\)', html)
        if m:
        ...
        if m:
            ...
            return f"__test={a * b if op == '*' else a + b}"
    ...
    async def _fetch_infinityfree_bypass(self, url: str) -> Optional[tuple]:
        """Cookie math bypass for InfinityFree (fast, no browser needed)."""
        ssl_ctx = create_ssl_context(verify=False)
        connector = TCPConnector(ssl=ssl_ctx, limit=10, force_close=True)
        timeout = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT + 30, connect=CONNECT_TIMEOUT)
        parsed = urlparse(url)
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                is_challenge = any(kw in first_html.lower() for kw in
                    ['__test', 'cheungid', 'tochk', 'checking your browser'])
                ...
                logger.info(f"InfinityFree cookie bypass: {cookie_val}")
                ...
                for attempt in range(3):
                    ...
                    async with session.get(
                        target, allow_redirects=True, max_redirects=10,
                        ...
                        still_challenge = any(kw in html2.lower() for kw in
            ...
            raise Exception(f"InfinityFree cookie bypass failed: {e}")

        return None

    async def _fetch_direct(self, url: str, ssl_verify: bool = True, ua: str = None) -> Optional[tuple]:
        """Direct HTTP fetch with retry logic."""
        ssl_ctx = create_ssl_context(verify=ssl_verify)
        connector = TCPConnector(ssl=ssl_ctx, limit=20, force_close=True)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(url, allow_redirects=True, max_redirects=15) as resp:
    ...
    async def _fetch_with_cookies(self, url: str) -> Optional[tuple]:
        """Fetch with cookie acceptance."""
        ssl_ctx = create_ssl_context(verify=False)
        connector = TCPConnector(ssl=ssl_ctx, limit=10, force_close=True)
        timeout = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT + 30, connect=CONNECT_TIMEOUT)
        parsed = urlparse(url)
        base = f"{parsed.scheme}://{parsed.netloc}"
        headers = build_headers()
        headers["Cookie"] = "cookieConsent=accepted; GDPR=1; session=active"
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                try:
                    async with session.get(base, allow_redirects=True, max_redirects=10):
                        pass
                except Exception:
                    pass
    ...
    async def _fetch_alt_ua(self, url: str) -> Optional[tuple]:
        ...
        for ua in USER_AGENTS:
            try:
                result = await self._fetch_direct(url, ssl_verify=False, ua=ua)
                if result:
                    return result
                await asyncio.sleep(0.5)
            except Exception:
    ...
    async def _fetch_google_cache(self, url: str) -> Optional[tuple]:
        ...
        try:
            async with aiohttp.ClientSession(
                connector=connector, timeout=timeout, headers=build_headers()
            ) as session:
                async with session.get(cache_url, allow_redirects=True) as resp:
                    if resp.status == 200:
                        ct = resp.headers.get("Content-Type", "")
                        text = await resp.text(errors='replace')
                        if text and len(text) > 100:
                            soup = BeautifulSoup(text, 'html.parser')
                            for d in soup.find_all(id='google-cache-hdr'):
                                d.decompose()
                ...
                async with session.get(api_url) as api_resp:
                    if api_resp.status == 200:
                        ...
                        if archived and archived.get("available"):
                            wb_url = archived["url"]
                            async with session.get(wb_url, allow_redirects=True) as page_resp:
                                if page_resp.status == 200:
                                    ct = page_resp.headers.get("Content-Type", "")
                                    text = await page_resp.text(errors='replace')
                                    if text and len(text) > 100:
...
class WebScraper:
    ...
    @staticmethod
    def _detect_source_file(url: str, content_type: str) -> Optional[str]:
        """
        Detect if URL points to a source code file (.py, .php, etc).
        Returns the file extension if it's a source file, else None.
        """
        ...
        if any(x in ct_lower for x in [
            'x-python', 'x-php', 'x-httpd-php',
            'x-ruby', 'x-go', 'x-java', 'shellscript',
        ]):
            # Guess extension from content-type
            if 'python' in ct_lower: return '.py'
            if 'php'    in ct_lower: return '.php'
            if 'ruby'   in ct_lower: return '.rb'
            if 'go'     in ct_lower: return '.go'
            if 'java'   in ct_lower: return '.java'
            if 'shell'  in ct_lower: return '.sh'
        return None
    ...
    async def scrape_single(
        ...
        # Fetch with 8-strategy fallback
        ...
        if not html:
            return None, (
                ...
                f"All 8 strategies failed.\n"
                f"Last error: <code>{method}</code>\n\n"
                "Strategies tried:\n"
                "playwright → infinityfree_cookie → direct_ssl\n"
                "direct_no_ssl → with_cookies → alt_ua\n"
                "google_cache → wayback_machine"
            )

        result.fetch_method = method

        # ── Source file shortcut (.py / .php / .js / etc.) ──────────────
        src_ext = self._detect_source_file(final_url, ct)
        if src_ext:
            import os
            fname = os.path.basename(urlparse(final_url).path) or f"source{src_ext}"
            ...
            result.total_size = len(html.encode('utf-8', errors='replace'))
            result.elapsed = time.time() - start_time
            if progress_callback:
                await progress_callback(
                    f"✅ <b>Source file fetched!</b>\n\n"
    ...
    async def scrape_all_pages(
        ...
        if not html:
            return None, (
                ...
                f"All 8 strategies failed.\n"
                f"Last error: <code>{method}</code>\n\n"
                "• If you see 'playwright' in the error above, "
                "Chromium may not be installed correctly.\n"
                "• Try /scrape (single page) first to test."
            )
