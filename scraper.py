...
class AdvancedFetcher:
    ...
    def _solve_infinityfree_token(html: str):
        """
        InfinityFree / epizy / wuaze hosts show an anti-bot page that
        sets a cookie via JavaScript. The JS does:
            var s = (number) + (number);
            document.cookie = "__test=" + s + "; path=/";
        We parse both numbers, add them, and return the cookie value.
        """
        import re as _re
        # Pattern 1: var s=NNN+NNN
        m = _re.search(r'var\s+s\s*=\s*(\d+)\s*\+\s*(\d+)', html)
        if m:
            return f"__test={int(m.group(1)) + int(m.group(2))}"
        # Pattern 2: "__test=" + (NNN+NNN)
        m = _re.search(r'"__test="\s*\+\s*\((\d+)\s*\+\s*(\d+)\)', html)
        if m:
            return f"__test={int(m.group(1)) + int(m.group(2))}"
    ...
    async def _fetch_infinityfree_bypass(self, url: str):
        """
        Bypass InfinityFree/epizy/wuaze.com anti-bot challenge.
        These hosts redirect bots to a JS challenge page that sets a
        __test cookie. We extract the cookie value and replay the request.
        """
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                # Step 1: first request — get the challenge page
                ...
                # If already returned real content, done
                if (first_status == 200 and len(first_html) > 500
                        and '__test' not in first_html
                        and 'checking your browser' not in first_html.lower()):
                    return first_html, 200, "text/html", first_url

                # Step 2: solve the __test token
                cookie_val = self._solve_infinityfree_token(first_html)
                if not cookie_val:
                    logger.warning("InfinityFree bypass: token not found in challenge page")
                    return None

                logger.info(f"InfinityFree bypass: solved → {cookie_val}")
                await asyncio.sleep(2.0)  # mimic JS execution delay

                # Step 3: replay with solved cookie
                ...
                for attempt in range(3):
                    async with session.get(
                        ...
                        if s2 == 200 and len(html2) > 500:
                            if '__test' not in html2 or len(html2) > 3000:
