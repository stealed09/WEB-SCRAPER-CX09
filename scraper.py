...
async def _fetch_with_playwright(url: str) -> Optional[tuple]:
    """
    Use Playwright headless browser to fetch pages that require JS execution.
    This is the ONLY reliable way to bypass InfinityFree/wuaze.com challenges.
    ...
    try:
        async with async_playwright() as pw:
            ...
            try:
                # Navigate and wait for network to be idle (JS challenge completes)
                ...
                # Extra wait for InfinityFree redirect (it waits ~2s then reloads)
                await page.wait_for_timeout(3000)

                # Wait again after potential redirect
                try:
                ...
                logger.info(f"Playwright: fetched {final_url}, size={len(html)}, status={status}")

                await browser.close()

                if html and len(html) > 500:
                    # Check it's not still a challenge page
                    low = html.lower()
                    if "cheungid" not in low and "tochk" not in low:
                        return html, status, ct, final_url
